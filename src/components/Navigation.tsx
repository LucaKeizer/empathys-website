      ...
      <nav className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            <div className="hidden lg:flex items-center space-x-1 ml-16">
              {navItems.map((item) => (
                <div key={item.href} className="relative group">
                  <Link
                    href={item.href}
                    className={`relative px-4 py-2 rounded-lg font-medium transition-all duration-300 inline-flex items-center gap-1 group ${
                      pathname === item.href 
                  ...
                  {item.hasDropdown && (
                    <div className="absolute top-full left-0 mt-2 w-64 bg-white/95 backdrop-blur-md rounded-xl shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 border border-gray-100">
                      <div className="py-2">
                        {dropdownItems.map((dropdownItem) => (
                          <Link
                            key={dropdownItem.href}
                            href={dropdownItem.href}
                            className={`block px-4 py-3 transition-all duration-200 hover:bg-gradient-to-r hover:from-teal-50 hover:to-blue-50 ${
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link 
              href="/winkelwagen" 
              className="relative p-3 text-gray-700 hover:text-teal-600 transition-all duration-300 hover:bg-white/60 rounded-lg group"
            >
            ...
            <button
              ...
              className="lg:hidden p-3 text-gray-700 hover:text-teal-600 transition-all duration-300 hover:bg-white/60 rounded-lg"
            >
